...
public class UserController {
    ...
    @PostMapping("/users")
    public ResponseEntity<PersonResponse> creatNewUser(@Valid @RequestBody PersonRequest personRequest) {
        return new ResponseEntity<PersonResponse>(
            personService.createNewUsers(personRequest),
            HttpStatusCode.valueOf(201)
        );
    }
    
    @PatchMapping("users/{id}")
    public ResponseEntity<PersonResponse> parcialUpdateRol(@PathVariable Long id, @RequestBody PersonRequest personDto){
